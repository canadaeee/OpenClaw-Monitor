...
1. OpenClaw node 与 Monitor 运行在同一台机器
2. node 本机输出 JSONL 事件
3. node-side collector 自动轮询并标准化入库
...
1. Run Monitor on the same machine as the OpenClaw node
2. Emit local JSONL events from the node
3. Let the node-side collector poll and normalize those events
