...
if port_in_use 12888; then
  echo "Port 12888 is already in use. Stop existing service with: bash ./scripts/stop-macos.sh"
  exit 1
fi

if port_in_use 12889; then
  echo "Port 12889 is already in use. Stop existing service with: bash ./scripts/stop-macos.sh"
  exit 1
fi
